  ...
  template: `
    <ion-header>
      <ion-toolbar color="primary">
        ...
        ></ion-buttons>
        <ion-title>Messages (Admin)</ion-title>
      </ion-toolbar>
    </ion-header>
      ...
      <ion-list>
        <ion-item *ngFor="let c of conversations" (click)="open(c)">
          <ion-label>
            <h3>{{ c.subject }}</h3>
            <p>
              User #{{ c.user_id }} • {{ c.status | titlecase }} • Updated
...
export class AdminMessagesPage {
  ...
  refresh(ev: any) {
    this.api.adminGetConversations().subscribe({
      next: (rows) => {
  ...
  open(c: any) {
    this.router.navigate(['/admin/messages', c.id]);
  }
}
